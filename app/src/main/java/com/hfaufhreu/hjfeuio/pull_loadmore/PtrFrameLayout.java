package com.hfaufhreu.hjfeuio.pull_loadmore;
...
import com.hfaufhreu.hjfeuio.pull_loadmore.indicator.PtrIndicator;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.DefaultLoadMoreFooter;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.GridViewHandler;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.ILoadViewMoreFactory;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.ILoadViewMoreFactory.ILoadMoreView;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.ListViewHandler;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.OnLoadMoreListener;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.OnScrollBottomListener;
import com.hfaufhreu.hjfeuio.pull_loadmore.loadmore.RecyclerViewHandler;
import com.hfaufhreu.hjfeuio.util.LogUtil;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(com.hfaufhreu.hjfeuio.R.styleable.PtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            LogUtil.d(LOG_TAG, "onMeasure frame: width: " + getMeasuredHeight() + ", height: " + getMeasuredWidth() + ", padding:" +
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                LogUtil.d(LOG_TAG, "onMeasure content, width: " + getMeasuredWidth() + ", height: " + getMeasuredHeight() + ", margin: " +
                        lp.leftMargin + lp.topMargin + lp.rightMargin + lp.bottomMargin);
                LogUtil.d(LOG_TAG, "onMeasure, currentPos: " + mPtrIndicator.getCurrentPosY() + ", lastPos: " +
                        mPtrIndicator.getLastPosY() + ", top: " + mContent.getTop());
            }
        }
    }
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                LogUtil.d(LOG_TAG, "onLayout header: " + left + top + right + bottom);
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                LogUtil.d(LOG_TAG, "onLayout content: " + left + top + right + bottom);
            }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mPtrIndicator.hasLeftStartPosition()) {
                    if (DEBUG) {
                        LogUtil.d(LOG_TAG, "call onRelease when user release");
                    }
                    onRelease(false);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    LogUtil.v(LOG_TAG, "ACTION_MOVE: offsetY:" + offsetY + ", currentPos: " + mPtrIndicator.getCurrentPosY() + ", moveUp: " + moveUp + ", canMoveUp: " + canMoveUp + ", moveDown: " + moveDown + ": canMoveDown: " + canMoveDown);
                }
    ...
    private void movePos(float deltaY) {
        ...
        if ((deltaY < 0 && mPtrIndicator.isInStartPosition())) {
            if (DEBUG) {
                LogUtil.e(LOG_TAG, String.format("has reached the top"));
            }
            return;
        }
        ...
        if (mPtrIndicator.willOverTop(to)) {
            if (DEBUG) {
                LogUtil.e(LOG_TAG, String.format("over top"));
            }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
                LogUtil.i(LOG_TAG, "PtrUIHandler: onUIRefreshPrepare, mFlag " + mFlag);
            }
        }
        ...
        if (DEBUG) {
            LogUtil.v(LOG_TAG, "updatePos: change: " + change + ", current: " + mPtrIndicator.getCurrentPosY() + " last: "
                    + mPtrIndicator.getLastPosY() + ", top: " + mContent.getTop() + ", headerHeight: %s" + mHeaderHeight);
        }
    ...
    public void setRefreshCompleteHook(PtrUIHandlerHook hook) {
        ...
        hook.setResumeAction(new Runnable() {
            ...
            public void run() {
                if (DEBUG) {
                    LogUtil.d(LOG_TAG, "mRefreshCompleteHook resume.");
                }
    ...
    private void performRefresh() {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            if (DEBUG) {
                LogUtil.i(LOG_TAG, "PtrUIHandler: onUIRefreshBegin");
            }
        }
        if (mPtrHandler != null) {
    ...
    private boolean tryToNotifyReset() {
        if ((mStatus == PTR_STATUS_COMPLETE || mStatus == PTR_STATUS_PREPARE) && mPtrIndicator.isInStartPosition()) {
            if (mPtrUIHandlerHolder.hasHandler()) {
                ...
                if (DEBUG) {
                    LogUtil.i(LOG_TAG, "PtrUIHandler: onUIReset");
                }
            }
    ...
    protected void onPtrScrollAbort() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "call onRelease after scroll abort");
            }
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "call onRelease after scroll finish");
            }
            onRelease(true);
        }
    }
    ...
    final public void refreshComplete() {
        if (DEBUG) {
            LogUtil.i(LOG_TAG, "refreshComplete");
        }
        ...
        if (delay <= 0) {
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "performRefreshComplete at once");
            }
            ...
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "performRefreshComplete after delay: " + delay);
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "performRefreshComplete do nothing, scrolling:" + mScrollChecker.mIsRunning + ", auto refresh: " + mFlag);
            }
            return;
        }
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mPtrIndicator.hasLeftStartPosition() && !ignoreHook && mRefreshCompleteHook != null) {
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "notifyUIRefreshComplete mRefreshCompleteHook run.");
            }
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            if (DEBUG) {
                LogUtil.i(LOG_TAG, "PtrUIHandler: onUIRefreshComplete");
            }
    ...
    private void sendCancelEvent() {
        if (DEBUG) {
            LogUtil.d(LOG_TAG, "send cancel event");
        }
    ...
    private void sendDownEvent() {
        if (DEBUG) {
            LogUtil.d(LOG_TAG, "send down event");
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        public void run() {
            ...
            if (DEBUG) {
                if (deltaY != 0) {
                    LogUtil.v(LOG_TAG, "scroll: " + finish + ", start: " + mStart + ", to: " + mTo + ", currentPos: " +
                            mPtrIndicator.getCurrentPosY() + ", current :" + curY + ", last: " + mLastFlingY + ", delta: " + deltaY);
                }
            }
            if (!finish) {
                mLastFlingY = curY;
        ...
        private void finish() {
            if (DEBUG) {
                LogUtil.v(LOG_TAG, "finish, currentPos:" + mPtrIndicator.getCurrentPosY());
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "tryToScrollTo: start: " + mStart + ", distance:" + distance + ", to:" + to);
            }
            removeCallbacks(this);
