...
public class GlodVipFragment extends BaseMainFragment {
    ...
    private void initView() {
        ...
        addFooterView();
        recyclerView.addItemDecoration(new DiamondItemDecoration((int) ScreenUtils.instance(getContext()).dip2px(3), list, App.role < 1));
        ...
        adapter.setOnClickGlodVipItemListener(new GlodVipAdapter.OnClickGlodVipItemListener() {
            ...
            public void onClickGlodVipItem(int position) {
                if (App.role == 0) {
                    DialogUtil.getInstance().showSubmitDialog(getContext(), false, "该片为会员视频，请开通会员后观看",
                            App.role, false, true, list.get(position).getVideo_id(), false, PageConfig.GLOD_POSITOTN_ID);
                } else {
    ...
    private void initHeaderView(final List<VideoInfo> headerLists) {
        ...
        headerView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (App.role == 0) {
                    DialogUtil.getInstance().showSubmitDialog(getContext(), false,
                            "该片为会员视频，请开通会员后观看", App.role, false, true, info.getVideo_id(), false, PageConfig.GLOD_POSITOTN_ID);
                } else {
                    toVideoDetail(info);
                }
            }
